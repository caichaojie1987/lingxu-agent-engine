...
lingxu 最初服务的业务，是房地产营销的"线索 → 成交 → 再引流"闭环——**前端引流找线索、
AI 机器人电销筛客、意向客户转人工接待、接待全程智能录音 + AI 转录、AI 分析客户抗性/意向/成交点、
按客户画像匹配 ERP 在售产品并给出针对性销售建议、客户沉淀进 CRM、项目负责人依据真实接待数据
制定新的方案与政策、再回到前端引流**。这个闭环里的每一个 AI 对话环节，都由本引擎驱动：

```mermaid
flowchart LR
    A[前端引流<br/>电Call·渠道·线上] --> B["AI 机器人电销<br/>(外呼对话大脑)"]
    B -->|意向客户| C["销售顾问接待<br/>(智能录音)"]
    C --> D["AI 转录分析<br/>(抗性·意向·成交点)"]
...
| 转录后分析 | 客户抗性、意向评级、成交点分析，结构化输出 | noTools 单发直答 |
